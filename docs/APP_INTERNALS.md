...
- UI (index.html #clipInspectorPanel):
  ...
  - KÉO ĐƯỢC hình thoi trên block để đổi THỜI ĐIỂM (2026-09-20, startKeyframeDrag/handleKeyframeDrag/finishKeyframeDrag). Bấm (không kéo, ngưỡng 3px) vẫn tua playhead tới keyframe như trước.
    - MỘT hình thoi = MỘT THỜI ĐIỂM (hợp mốc của mọi trục), nên cú kéo dời CẢ CỤM: transform + âm lượng + màu ('adj.*'). moveKeyframesAt duyệt theo khoá THẬT có trong obj.keyframes, không theo danh sách field cứng. Dời lẻ một trục là xé đôi keyframe Vị trí và phá bất biến ghép cặp X/Y.
    - KẸP giữa hai keyframe liền kề (chừa kfMinGap() = 1.5×KF_EPS), KHÔNG đi qua được — đúng nếp Premiere. Cho đi qua thì phải chọn giữa "gộp" (nuốt mất keyframe của người dùng ngay giữa cú kéo) và "đổi chỗ" (đường chuyển động lật ngược); kẹp là lựa chọn duy nhất không mất dữ liệu.
    - Bắt dính (khi Bắt dính đang bật) vào playhead + hai mép block, KHÔNG bắt dính vào keyframe khác (chúng đã là biên của cú kéo). Mốc nằm ngoài [lo,hi] bị bỏ qua.
    - Dữ liệu sửa NGAY trong lúc kéo để preview đổi theo tay; recordHistory() chạy ở bước dịch chuyển ĐẦU TIÊN nên Ctrl+Z trả về trạng thái trước cả cú kéo. Timeline được dựng lại mỗi bước nên "cái đang kéo" nhận ra bằng (ownerKey, currentT) chứ không giữ tham chiếu phần tử. Mốc làm tròn về mili giây (kfRound) — phép đổi pixel→giây sinh rác dấu phẩy động đi thẳng vào .crab.
    - CHỖ CHẾT LẶNG: handler mousedown của #segmentsTrack (index.html) chạy ở pha CAPTURE và stopPropagation ở nhánh lane chính, nên nó phải NHƯỜNG `.editing-kf-marker` — quên là kéo keyframe trên LANE CHÍNH không chạy trong khi lane overlay vẫn ngon. Ngược lại chính marker phải stopPropagation (trước cả khi xét lane khoá) để cú kéo không biến thành kéo block. Lane khoá: không kéo được, bấm vẫn tua.
    - Test: `npm run test:keyframe-drag`.
